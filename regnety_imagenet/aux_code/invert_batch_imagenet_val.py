...
Invert ImageNet-1K samples from regnet_y_3_2gf trunk-output features via gradient-based optimization.
...
def tv_loss(x: torch.Tensor, per_sample: bool = False) -> torch.Tensor:
    ...
    if per_sample:
        ...
        return dh.abs().sum() + dw.abs().sum() + dhr.abs().sum() + dhb.abs().sum() + dwr.abs().sum() + dwb.abs().sum()
...
def gray_edge_l1(rgb, per_sample=False):  # rgb: (B,3,H,W) already ImageNet-normalized
    dx = rgb[:,:,:,1:] - rgb[:,:,:,:-1]   # (B,3,H,W-1)
    dy = rgb[:,:,1:,:] - rgb[:,:,:-1,:]   # (B,3,H-1,W)

    def dev_from_mean(d):
        m = d.mean(dim=1, keepdim=True)      # (B,1,*,*)
        return (d - m).abs().sum(dim=1)      # L1 over channels -> (B,*,*)
...
def compute_all_losses(feat, target_feat, x, tv_weight, l2_weight=0.0, l1_weight=0.0, tv_loss_orig=None, per_sample=False):
    ...
    if per_sample:
        loss_mse = l2_weight * F.mse_loss(feat.mean(dim=[1,2,3], keepdim=True), target_feat.mean(dim=[1,2,3], keepdim=True), reduction='none').sum(dim=[1,2,3])
    else:
        loss_mse = l2_weight * F.mse_loss(feat.mean(dim=[1,2,3], keepdim=True), target_feat.mean(dim=[1,2,3], keepdim=True))
    
    # TV loss with threshold: max(tv_loss(x) - 0.1*tv_loss_orig, 0)
    tv_x = tv_loss(x, per_sample=per_sample)
